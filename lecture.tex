...
\begin{frame}{Propagation}
  \begin{itemize}
  \item We can traverse the DAG to compute the chain rule
    \begin{enumerate}
    \item Root node initialized with $\frac{df}{df} = 1$
    \item Other nodes accumulate values from all parents, scaled according to
      edge weights.
    \end{enumerate}
  \item Note, we have to traverse in DAG-order, and we have to
    visit all edges.
  \end{itemize}
\end{frame}
